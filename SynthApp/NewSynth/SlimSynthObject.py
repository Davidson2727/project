...
class SlimSynthObject(file):
    ...
    def getVoiceFilters(self):
        return self._voiceFilters
    ...
    def loadLocal(self, _input):
        ...
        self.load(_input)
        for i in range(Nums.TRANSZERO.value,Nums.TRANSTHREESIX.value,Nums.TRANSTWELVE.value):
            self._temp1[i // Nums.TRANSTWELVE.value] = int(self._voices[i + Nums.TRANSONE.value])
        self._voices = self._temp1
        for i in range(Nums.TRANSZERO.value, Nums.TRANSTHIRTEEN.value, Nums.TRANSTHREE.value):
            self._temp2[Nums.TRANSZERO.value][i // Nums.TRANSTHREE.value] = int(self._voiceFilters[i + Nums.TRANSTWO.value])
            self._temp2[Nums.TRANSONE.value][i // Nums.TRANSTHREE.value] = int(self._voiceFilters[i + Nums.TRANSTWONINE.value])
            self._temp2[Nums.TRANSTWO.value][i // Nums.TRANSTHREE.value] = int(self._voiceFilters[i + Nums.TRANSFIVESIX.value])
    ...
    def setVoices(self, _input):
        self._voices = _input


    def setVoiceFilters(self, _input):
        self._voiceFilters  = _input


    def setWaves(self, _input):
        self._voices  = _input


    def setWaveFilters(self, _input):
        self._voiceFilters  = _input
